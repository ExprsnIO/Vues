...
interface DomainCluster {
  ...
  cluster: {
    ...
    status: 'active' | 'draining' | 'offline' | 'error' | 'inactive';
    region: string | null;
    endpoint?: string;
    gpuEnabled: boolean;
    gpuCount: number;
...
export default function DomainClustersPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'offline': return 'bg-orange-500/10 text-orange-400 border-orange-500/30';
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'docker-swarm': return 'Docker Swarm';
      default: return type;
    }
  };
      ...
      {isDev && (
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-blue-400">PLC Directory</h3>
              <p className="text-sm text-blue-300 mt-1">
                Local development PLC server: <span className="font-mono">{`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3002'}/plc`}</span>
              </p>
              <p className="text-xs text-blue-300/70 mt-1">
                Visit <a href="/admin/plc" className="underline hover:text-blue-200">Admin → PLC Directory</a> to manage identities
              </p>
            </div>
          </div>
        </div>
        ...
        {clusters.map((dc: DomainCluster) => (
          <div key={dc.id} className="bg-surface border border-border rounded-lg p-4">
            ...
            <div className="flex gap-2 mb-3">
              {dc.cluster.tls?.enabled && (
                <span className="px-2 py-0.5 text-xs bg-green-500/10 text-green-400 rounded-full flex items-center gap-1">
              ...
              {dc.cluster.auth?.type && dc.cluster.auth.type !== 'none' && (
                <span className="px-2 py-0.5 text-xs bg-blue-500/10 text-blue-400 rounded-full flex items-center gap-1">
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M18 8a6 6 0 01-7.743 5.743L10 14l-1 1-1 1H6v2H2v-4l4.257-4.257A6 6 0 1118 8zm-6-4a1 1 0 100 2 2 2 0 012 2 1 1 0 102 0 4 4 0 00-4-4z" clipRule="evenodd" />
                  </svg>
                  {dc.cluster.auth.type}
                </span>
              )}
            </div>

            {dc.cluster.errorMessage && (
              <div className="mb-3 p-2 bg-red-500/10 border border-red-500/20 rounded text-xs text-red-400">
                {dc.cluster.errorMessage}
              </div>
            )}

            {dc.cluster.lastHealthCheck && (
              <div className="mb-3 text-xs text-text-muted">
                Last check: {new Date(dc.cluster.lastHealthCheck).toLocaleString()}
              </div>
            )}
              ...
              >
                ...
                {availableClusters.map((cluster: AvailableCluster) => (
                  <option key={cluster.id} value={cluster.id}>
                    {cluster.name} ({cluster.type.replace('-', ' ')}{cluster.region ? ` - ${cluster.region}` : ''})
                    {cluster.gpuEnabled ? ' [GPU]' : ''}
                    {cluster.tls?.enabled ? ' [TLS]' : ''}
                  </option>
                ))}
              </select>
            </div>
