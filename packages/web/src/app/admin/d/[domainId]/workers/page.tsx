...
export default function DomainWorkersPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin', 'domain', domainId, 'workers', 'list'],
    queryFn: () => api.getWorkerQueues(domainId),
    enabled: !!domainId,
  ...
  const pauseMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'domain', domainId, 'workers'] }),
  });
      ...
      <div className="bg-surface border border-border rounded-xl overflow-hidden">
        ...
        {queues.length === 0 ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-text-muted text-sm">No queues configured for this domain</p>
          </div>
        ) : (
