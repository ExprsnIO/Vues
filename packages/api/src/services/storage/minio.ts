...
export class MinIOProvider implements StorageProvider {
  readonly name = 'MinIO / Self-hosted S3';
  ...
  constructor(config: StorageConfig) {
    if (!config.minio) {
      throw new Error('MinIO configuration is required for MinIO provider');
    }
    ...
    // Parse endpoint to determine if it's HTTPS
    const url = new URL(this.endpoint);
    const useSSL = config.minio.useSSL || url.protocol === 'https:';

    this.client = new S3Client({
      endpoint: this.endpoint,
      region: config.region || 'us-east-1', // MinIO requires a region, even if unused
      credentials: {
  ...
  /**
   * Create the bucket if it doesn't exist (MinIO-specific helper)
   */
